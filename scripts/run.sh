...
DEFAULT_TOPIC="An environment setup validator that checks if a developer's machine has all required tools and versions for a project. Read requirements from a manifest file and report what is missing or outdated."
DEFAULT_CONTEXT="Support checking for CLI tools, language runtimes, package managers, and running services (databases, etc.)."
